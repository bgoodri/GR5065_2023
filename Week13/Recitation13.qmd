...
X <- model.matrix(cbind(y, n - y) ~ 
                    gender + race + income + rms::rcs(age), data = dataset)
X <- X[ , -1]
...
How would you draw outcomes? Hint: Utilize `group_by(dataset, year)`
:::
:::
:::
...
group_by(dataset, year) %>% 
    summarize(eta = rep(draws$gamma + rnorm(R, sd = draws$sigma_a), 
                        each = n()) + # ^ these are draws of a_j
                c(t(draws$beta %*% 
                    t(X[dataset$year == first(year), ]))),
              mu = plogis(eta),
              y = rbinom(R * n(), size = rep(n, R), mu),
              .groups = "drop")
```
...
prior <- stan_glmer(cbind(y, n - y) ~ 
                    ...
                    family = binomial) # default priors
...
-   The $13$ $a_j$ parameters do not count as $13$ effective parameters because they have a $\mathcal{N}\left(0,\sigma_a\right)$ prior and the posterior distribution of $\sigma_a$ is not huge. This is what makes hierarchical models ideal for predicting a future "big unit" (election in this case).
