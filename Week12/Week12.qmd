...
outcomes <- rowwise(draws) %>% 
  summarize(eta = gamma + beta_1 * X[ , 1] + 
              beta_2 * X[ , 2] + beta_3 * X[ , 3],
            mu = plogis(eta),
            Urban_Density = poll_grouped$Urban_Density,
            y = rbinom(nrow(X), prob = mu,
                       size = poll_grouped$observations)) %>% 
  ungroup

ggplot(outcomes) + # plot on next slide
  geom_boxplot(aes(x = Urban_Density, y = mu))
```

## Plot from Previous Slide

```{r}
ggplot(outcomes) + geom_boxplot(aes(x = Urban_Density, y = mu))
```
...
draws <- tibble(
  ...
  zeta_2 = rnorm(R, m0, s0),
  zeta_3 = rnorm(R, m0, s0),
  zeta_4 = rnorm(R, m0, s0)
) %>% 
  rowwise %>% 
  summarize( # x is centered
    zeta = sort(zeta),
