...
```
-   Wells in many countries have water that is contaminated with arsenic
...
as_tibble(wells) # outcome is switch while dist is in meters (bad)
X <- model.matrix(switch ~ arsenic * I(dist / 100), data = wells)
X <- X[ , -1] # drop (Intercept) that is included by default
x_bar <- colMeans(X)
# subtract average from each of the three columns
...
post <- stan_glm(switch ~ arsenic * I(dist / 100),   # interaction  
                 data = wells, # inside I() is interpreted as math
                 family = binomial(link = "probit"), # not default
...
```{r}
ggplot(draws) + geom_density(aes(mu)) + facet_wrap(~ arsenic + dist)
```
...
-   In order for $\mu$ to be reasonable with a nonlinear inverse link function, $\eta$ usually needs to be in a small subset of $\mathbb{R}$, which implies $\gamma \approx g\left(\mathbb{E}Y\right)$ and $\beta_k$ can't be too large in magnitude
