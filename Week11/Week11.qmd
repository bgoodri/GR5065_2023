...
```{r}
my_prior <- prior(normal(-1, 1), class = "b") + 
...
outcomes <- 
  group_by(pars, j) %>% 
  rowwise %>% 
  summarize(eta = gamma + beta * x,   # x has been centered
...
-   Called categorical, although ML refers to it as multinomial
...
#| cache: true
#| results: hide
post <- brm(Species ~ ., data = iris, family = categorical,
            prior = my_prior, save_pars = save_pars(all = TRUE))
```
